...
from app.database.database import insert_assistant_data, insert_assistant_threads, get_threads_for_assistant, update_assistant_data, get_all_assistants
from app.services.assistant import create_assistant, update_assistant_details, create_assistant_with_file
from app.services.threads import create_thread , add_message_to_thread
...
async def handle_form(
    ...
    files_list = await create_files(file_inputs)
    ...
    try:
        ...
        print("1")
        if file_ids:
            assistant = await create_assistant_with_file(file_inputs, assistant_config)  # Await the asynchronous function
            print("2")
    ...
    print(f"Assistant: {company_name}, File Names: {[file_input.filename for file_input in file_inputs] if file_inputs else 'N/A'}, OpenAI File IDs: {file_ids if file_ids else 'N/A'}, Thread ID: {thread_id}")
...
async def process_message(request: Request):
    ...
    try:
        response_text = await add_message_to_thread(thread_id, user_message, assistant_id)
    except Exception as e:
